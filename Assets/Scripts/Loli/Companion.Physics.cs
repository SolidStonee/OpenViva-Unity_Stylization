...
namespace viva
{
    ...
    public partial class Companion : Character
    {
        ...
        public override void OnCharacterCollisionEnter(CharacterCollisionCallback ccc, Collision collision)
        {
            ...
            //TODO: Convert ModelRebuilder to function to create companion ONLY when it's ready
            if (active == null)
            {
                return;
            }
            ...
            if (!hasBalance || isNotFoot)
            {
                if (!active.horseback.isOnHorse)
                {
                    if (collision.relativeVelocity.sqrMagnitude > 36.0f)
                    {
                        ...
                        PlayPhysicsCollisionSound(GameDirector.instance.companionSettings.bodyImpactHardSound.GetRandomAudioClip(), true);
                    }
                    else if (collision.relativeVelocity.sqrMagnitude > 12.0f)
                    {
                        PlayPhysicsCollisionSound(GameDirector.instance.companionSettings.bodyImpactSoftSound.GetRandomAudioClip(), false);
                    }
                }
            }
