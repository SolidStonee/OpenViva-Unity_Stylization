...
namespace Viva
{
    ...
    public abstract class HandState : OccupyState
    {
        ...
        public void GrabItemRigidBody(Item targetItem)
        {
            ...
            BeginRigidBodyGrab(targetItem.rigidBody, selfItem.rigidBody, targetItem.settings.usesFixedJoint, HoldType.OBJECT, 0.6f);
        ...
        public void GrabGenericRigidBody(Collider collider, Vector3 pos, Vector3 normal)
        {
            ...
            else
            {
                ...
                if (targetBody == null)
                {
                    ...
                    Debug.LogError($"[HandState] Grab collider that failed was: {collider.name}");
                    return;
                }
                OnPreApplyHoldingTransform(targetItem);
