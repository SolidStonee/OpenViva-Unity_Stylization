...
public class DynamicBone : MonoBehaviour
{
    ...
    public Vector3 GetParticlePosition(int index) {
        foreach (ParticleTree pt in m_ParticleTrees) {
            if (index < pt.m_Particles.Count) {
                return pt.m_Particles[index].m_Position;
            }
            index -= pt.m_Particles.Count;
        }
        // If we've exhausted all ParticleTrees and the index is still not found,
        // it means the index was out of bounds.
        return Vector3.zero;
    }
    ...
    public int GetParticleCount() {
        int totalCount = 0;
        foreach (ParticleTree pt in m_ParticleTrees) {
            totalCount += pt.m_Particles.Count;
        }
        return totalCount;
    }

    public void FreezeAllParticles( float strength ){
        for (int i = 0; i < m_ParticleTrees.Count; ++i){
            
            ParticleTree pt = m_ParticleTrees[i];
            for (int j = 0; j < pt.m_Particles.Count; ++j)
            {
                pt.m_Particles[j].m_PrevPosition = Vector3.LerpUnclamped(pt.m_Particles[j].m_PrevPosition, pt.m_Particles[j].m_Position, strength);
            }
        }
    }
