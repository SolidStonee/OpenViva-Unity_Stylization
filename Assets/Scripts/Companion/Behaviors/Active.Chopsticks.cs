    ...
    public class ChopsticksBehavior : ActiveBehaviors.ActiveTask
    {
        ...
        protected class UserInfo
        {
            ...
            public bool AttemptRedistributeFingers()
            {
                ...
                if (realLeftCount == realRightCount)
                { //can't if both hands are the same
                    return false;
                }
                if (Mathf.Abs(realLeftCount - realRightCount) == 1)
                { //can't mirror
                    return false;
                }
        ...
        public void WaitUntilTapHandLoliTurn()
        {
            ...
            if (killMove)
            {   //prioritize removing GameDirector.player hand moves
        ...
        public void ChoseRandomMoveAvailable()
        {   //4 possibilities
